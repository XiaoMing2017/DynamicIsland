...
import com.dynamicisland.service.IslandAccessibilityService
...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupButtons() {
        ...
        b.btnStart.setOnClickListener {
            ...
            if (!isAccessibilityEnabled())       { toast("请先开启无障碍权限（获取音乐/来电信息必需）"); return@setOnClickListener }
        ...
        b.btnTest.setOnClickListener {
            ...
            val cases = listOf(
                ...
                arrayOf("com.android.incallui",   "来电",      "李四",                 "", true, false),
                arrayOf("com.tencent.mobileqq",   "QQ",        "群消息 (5)",            "王五: 今晚打游戏吗", false, false),
                arrayOf("com.autonavi.minimap",   "高德地图",  "前方300m左转",          "导航中", false, false)
            )
    ...
    private fun slider(bar: SeekBar, label: TextView, current: Int, min: Int, max: Int,
                       name: String, unit: String, save: (Int) -> Unit) {
        bar.max = max - min; bar.progress = current - min
        label.text = "$name：$current$unit"
        bar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(sb: SeekBar, v: Int, u: Boolean) {
                val real = v + min; label.text = "$name：$real$unit"
                save(real); sendBroadcast(Intent(FloatingService.ACTION_UPDATE_PREFS))
            }
    ...
    private fun updateStatus() {
        ...
        val n = isNLEnabled()
        val a = isAccessibilityEnabled()

        fun colorOf(ok: Boolean) = if (ok) 0xFF27AE60.toInt() else 0xFFE74C3C.toInt()

        b.statusOverlay.text = if (o) "✓ 悬浮窗" else "✗ 悬浮窗"
        b.statusOverlay.setTextColor(colorOf(o))
        b.statusNotification.text = if (n) "✓ 通知监听" else "✗ 通知监听"
        b.statusNotification.setTextColor(colorOf(n))
        b.statusAccessibility.text = if (a) "✓ 无障碍" else "✗ 无障碍"
        b.statusAccessibility.setTextColor(colorOf(a))

        // Start button active only when all 3 are granted
        b.btnStart.isEnabled = o && n && a
        b.btnStart.backgroundTintList = android.content.res.ColorStateList.valueOf(
            if (o && n && a) 0xFF111111.toInt() else 0xFFCCCCCC.toInt()
        )
    }
